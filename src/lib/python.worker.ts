...
class MypyWebworkerInterface {
    ...
    public async writeFile(location: string, input: string) {
        Sentry.addBreadcrumb({category: "data", message: `Writing file to ${location}`, level: "info"});
    ...
    public async runMypy(args: string[]): Promise<[string, string, number]> {
        ...
        Sentry.addBreadcrumb({category: "mypy", message: `Running mypy with args ${args.join(" ")}`, level: "info"});
    ...
    public async installPackage(package_name: string): Promise<string|null> {
        ...
        logBreadcrumb({category: "packages", message: `Successfully installed package ${package_name}`, level: "info"});
        return null;
    }
}
